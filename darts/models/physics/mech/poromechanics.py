...
class Poromechanics:
    ...
    """
    def __init__(self, timer, physics_filename, n_points, min_p, max_p, max_u, isMP = False, with_gpu = False, static_itor=False):
        """"
        ...
        #if grav:
        #    engine_name = eval("engine_nc_pm_%s%d_%d" % (plat, self.n_components, self.n_phases))
        #    acc_flux_etor_name = dead_oil_acc_flux_capillary_evaluator
        #    self.n_ops = self.n_components + self.n_components * self.n_phases + \
        #                 self.n_phases + self.n_phases + self.n_dim
        #else:
        #    if isMP:
        #        engine_name = eval("engine_nc_pm_%s%d" % (plat, self.n_components))
        #    else:
        engine_name = eval("engine_nc_pm_%s%d" % (plat, self.n_components))
        acc_flux_etor_name = pm_acc_flux_evaluator
        self.n_ops = 2 * (self.n_components + self.n_dim)
        ...
        if grav:
            ...
            self.acc_flux_etor = acc_flux_etor_name(self.el_dens_ev, self.rock_compaction_evaluator)
            #try:
                # try first to create interpolator with 4-byte index type
            self.acc_flux_itor = acc_flux_itor_name(self.acc_flux_etor,
                                    index_vector([n_points, n_points, n_points, n_points]),
                                    value_vector([-max_u, -max_u, -max_u, min_p]),
                                    value_vector([max_u, max_u, max_u, max_p]))
            #except RuntimeError:
